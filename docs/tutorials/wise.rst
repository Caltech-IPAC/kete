...
A field of view is a patch of sky as seen from an observer. apohele supports downloading
...
.. code-block:: python
    ...
    fovs = apohele.wise.fetch_WISE_fovs('Reactivation_2023')
...
.. code-block:: python
    ...
    # minute to download. Note that apohele caches these files after the first download.
    # Future queries will use the cached copy unless `force_download=True` is set.
    orb_table = apohele.mpc.fetch_known_orbit_data()

    # Now we can down select these to just NEOs:
    is_neo = apohele.population.neo(orb_table['peri_dist'], orb_table['ecc'])
...
.. code-block:: python
    ...
    states = apohele.propagate_n_body(states, jd, include_asteroids=True)
...
.. code-block:: python
    ...
    visible = apohele.fov_state_check(states, fovs)
...
.. note::
    ...
    ``apohele.SimultaneousStates.save_list(visible, "visible_wise_2023.bin")``

    The states may later be loaded using:

    ``visible = apohele.SimultaneousStates.load_list("visible_wise_2023.bin")``
...
.. code-block:: python
    ...
    for vis in visible[:n_show]:
        for state in vis:
            ...
            mjd = apohele.Time(vis.fov.observer.jd).mjd
