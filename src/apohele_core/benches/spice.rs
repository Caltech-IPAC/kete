...
use apohele_core::{spice::get_spk_singleton, state::State};
...
fn spice_get_state(jd: f64) {
    ...
    for _ in 0..1000 {
        let _ = spice
            .try_get_state(5, jd, 10, apohele_core::frames::Frame::Ecliptic)
            .unwrap();
    }
}
...
pub fn spice_benchmark(c: &mut Criterion) {
    let spice = get_spk_singleton().try_read().unwrap();
    let state = spice
        .try_get_state(5, 2451545.0, 10, apohele_core::frames::Frame::Ecliptic)
        .unwrap();
