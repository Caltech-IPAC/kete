//! Core apohele library code, which are wrappers over the apohele_core rust package.
...
// wrapper over [`apohele_core::state::State`]. These allow rust objects to exist in
// the python while still being reasonably fast. These wrappers also frequently expose
// the binary save/loading features written into the apohele_core code. Typically these
