...
def _validate_time(time: Union[float, Time]) -> float:
    ...
    :class:`~apohele.time.Time` object.
...
def get_state(
    ...
    """
    Calculates the :class:`~apohele.State` of the target object at the
    specified time `jd`.
    ...
    target:
        ...
        :meth:`~apohele.spice.loaded_objects`
    jd:
        Julian time (TDB) of the desired record.
    center:
...
def name_lookup(name: Union[int, str]) -> tuple[str, int]:
    ...
    >>> apohele.spice.name_lookup("jupi")
    ('jupiter barycenter', 5)

    >>> apohele.spice.name_lookup(10)
    ('sun', 10)
    ...
    >>> apohele.spice.name_lookup("Earth")
    ('earth', 399)

    >>> apohele.spice.name_lookup("Earth b")
    ('earth barycenter', 3)
...
def kernel_reload(filenames: Optional[list[str]] = None, include_cache=False):
    ...
    include_cache:
        This decides if all of the files contained within the apohele cache should
...
def earth_pos_to_ecliptic(
    ...
    website. The file provided in apohele is valid from ~2000 to ~2024. This file
...
def moon_illumination_frac(jd: Union[float, Time], observer: str = "399"):
    ...
    >>> apohele.spice.moon_illumination_frac(Time.from_ymd(2024, 2, 24))
    0.9964936478732302
