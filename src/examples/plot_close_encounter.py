...
obj = apohele.HorizonsProperties.fetch("'Oumuamua")
jd_start = apohele.Time.from_ymd(2016, 1, 1).jd
jd_end = apohele.Time.from_ymd(2018, 1, 1).jd
...
for jd in jds:
    last_state = apohele.propagate_two_body([last_state], jd)[0]
    obj_pos.append(last_state.pos)
    earth_pos.append(apohele.spice.get_state("Earth", jd).pos)
    earth_r.append(apohele.Vector(obj_pos[-1] - earth_pos[-1]).r)
...
for i, planet in enumerate(["Mercury", "Venus", "Earth", "Mars", "Jupiter"]):
    states = [apohele.spice.get_state(planet, jd).pos for jd in jds]
    pos = np.array(states).T
    ax.plot(pos[0], pos[1], pos[2], color=f"C{i}", alpha=0.5)
    pos = apohele.spice.get_state(planet, jd_center).pos
