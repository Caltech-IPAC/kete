...
# Inputs:
# -------
obj = apohele.HorizonsProperties.fetch("Eros", update_name=False)

start_time = apohele.Time.from_ymd(2023, 11, 1).jd
end_time = apohele.Time.from_ymd(2024, 11, 1).jd
...
state = apohele.propagate_n_body([obj.state], start_time)[0]
...
for t in times:
    ...
    state = apohele.propagate_n_body([state], t)[0]
    sun2obs = apohele.spice.mpc_code_to_ecliptic(site, t).pos
    earth2obs = apohele.spice.mpc_code_to_ecliptic(site, t, center="399").pos
    ...
    if obj.h_mag is not None:
        mag = apohele.flux.hg_apparent_mag(
...
dates = [apohele.Time(t).to_datetime() for t in times]
...
for t in day_steps:
    ...
    state = apohele.propagate_n_body([state], t + substep_time / 2)[0]
    nights = []
    for subrange in substeps:
        approx_state = apohele.propagate_two_body([state], t + subrange)[0]
        sun2obs = apohele.spice.mpc_code_to_ecliptic(site, t + subrange).pos
        earth2obs = apohele.spice.mpc_code_to_ecliptic(
...
dates = [apohele.Time(t + step_size / 2).to_datetime() for t in day_steps]
