use kete_core::spice::{try_name_from_id, SPK_SINGLETON};
...
pub fn spk_load_py(py: Python<'_>, filenames: Vec<String>) -> PyResult<()> {
    let singleton = &mut SPK_SINGLETON.write().unwrap();
...
pub fn spk_available_info_py(naif_id: i64) -> Vec<(f64, f64, i64, PyFrames, i32)> {
    let singleton = &SPK_SINGLETON.try_read().unwrap();
    singleton
...
pub fn spk_loaded_objects_py() -> Vec<i64> {
    let spk = &SPK_SINGLETON.try_read().unwrap();
...
#[pyo3(name = "spk_reset")]
pub fn spk_reset_py() {
    let spk = &mut SPK_SINGLETON.write().unwrap();
    spk.reset();
}
