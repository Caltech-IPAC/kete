...
use kete_core::spice::{PCK_SINGLETON, SPK_SINGLETON};
...
pub fn pck_load_py(filenames: Vec<String>) -> PyResult<()> {
    let singleton = &mut PCK_SINGLETON.write().unwrap();
...
pub fn pck_earth_frame_py(
    ...
    let pcks = &PCK_SINGLETON.try_read().unwrap();
    ...
    let spks = &SPK_SINGLETON.try_read().unwrap();
...
#[pyo3(name = "pck_reset")]
pub fn pck_reset_py() {
    let pck = &mut PCK_SINGLETON.write().unwrap();
    pck.reset()
}
