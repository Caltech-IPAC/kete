...
def get_state(
    ...
    target:
        ...
        :meth:`~neospy.spice.loaded_objects`
    jd:
        Julian time (TDB) of the desired record.
    center:
...
def name_lookup(name: Union[int, str]) -> tuple[str, int]:
    ...
    >>> neospy.spice.name_lookup("jupi")
    ('jupiter barycenter', 5)

    >>> neospy.spice.name_lookup(10)
    ('sun', 10)
    ...
    >>> neospy.spice.name_lookup("Earth")
    ('earth', 399)

    >>> neospy.spice.name_lookup("Earth b")
    ('earth barycenter', 3)
...
def moon_illumination_frac(jd: Union[float, Time], observer: str = "399"):
    ...
    >>> neospy.spice.moon_illumination_frac(Time.from_ymd(2024, 2, 24))
    0.9964936478732302
